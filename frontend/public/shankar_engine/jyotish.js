...
// Nepali mapping for planets, rashis, nakshatras, and key terms (shared with sjp.js)
window.PLANET_NEPALI = {
  'Sun': 'सूर्य', 'Surya': 'सूर्य',
  'Moon': 'चन्द्रमा', 'Chandra': 'चन्द्रमा',
  'Mars': 'मंगल', 'Mangala': 'मंगल',
  'Mercury': 'बुध', 'Budha': 'बुध',
  'Jupiter': 'गुरु', 'Guru': 'गुरु', 'Brihaspati': 'गुरु',
  'Venus': 'शुक्र', 'Shukra': 'शुक्र',
  'Saturn': 'शनिः', 'Shani': 'शनिः',
  'Rahu': 'राहु',
  'Ketu': 'केतु',
  'North Node': 'राहु',
  'South Node': 'केतु',
...
window.toNepaliPlanet = function(name) { return window.PLANET_NEPALI[name.trim()] || name; };
window.toNepaliRashi = function(name) { return window.RASHI_NEPALI[name.trim()] || name; };
window.toNepaliNakshatra = function(indexOrName) {
  if (typeof indexOrName === 'number') return window.NAKSHATRA_NEPALI[indexOrName] || indexOrName;
  const map = {
    'Ashwini': 0, 'Bharani': 1, 'Krittika': 2, 'Rohini': 3, 'Mrigashira': 4, 'Ardra': 5, 'Punarvasu': 6, 'Pushya': 7, 'Ashlesha': 8,
    'Magha': 9, 'Purva Phalguni': 10, 'Uttara Phalguni': 11, 'Hasta': 12, 'Chitra': 13, 'Swati': 14, 'Vishakha': 15, 'Anuradha': 16, 'Jyeshtha': 17,
    'Mula': 18, 'Purva Ashadha': 19, 'Uttara Ashadha': 20, 'Shravana': 21, 'Dhanishta': 22, 'Shatabhisha': 23, 'Purva Bhadrapada': 24, 'Uttara Bhadrapada': 25, 'Revati': 26
  };
  const idx = map[indexOrName.trim()];
  return typeof idx === 'number' ? window.NAKSHATRA_NEPALI[idx] : indexOrName;
};
